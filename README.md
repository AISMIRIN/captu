...
- **検索・絞り込み** — ARIB字幕テキスト検索、タグ/フィルタ (タイトル・話数・日付・複数タグ AND) による絞り込み
- **自動取り込み** — EPGメタデータ抽出、差分/並列/再取り込み、定期スケジューラ、削除・移動追跡
- **コンタクトシート** — 字幕PNGオーバーレイ、インターレース解除、小表示/フル解像度分離、フレーム選択永続化
- **JPEG取得** — 字幕あり/なしの切り替え (ブラウザ側合成)、Web Share API (スマホ) / クリップボードコピー (PC) / ダウンロード

**未実装 (将来):** LLM AI検索、マルチモーダル
